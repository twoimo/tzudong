...
│   │  │ NCP 지오코딩 + 20m 검증                                       │   │   │
│   │  │         ↓                                                    │   │   │
│   │  │ Gemini CLI → youtuber_review 추출 (3회 재시도)                 │   │   │
│   │  └──────────────────────────────────────────────────────────────┘   │   │
│   │         ↓ 성공                    ↓ 실패                            │   │
...
| 소스 | 파일 |
|------|------|
| URLs | `urls/urls.txt` (코드 236-252줄) |
| 이전 메타 | `meta/{video_id}.jsonl` (코드 91-101줄) |
...
| 소스 | 파일 | 용도 |
|------|------|------|
| meta | `meta/{video_id}.jsonl` | description에서 지도 URL 추출 (코드 785-791줄) |
| transcript | `transcript/{video_id}.jsonl` | recollect_version 저장 (코드 800-808줄) |
...
| 소스 | 파일 | 용도 |
|------|------|------|
| meta | `meta/{video_id}.jsonl` | title, recollect_id (코드 282-285줄) |
| transcript | `transcript/{video_id}.jsonl` | 자막 텍스트 (코드 288-298줄) |
...
### 필수 필드 검증 (parse_result.py 85-94줄)
```python
...
### 출력 구조 (parse_result.py 118-131줄)
```json
{
  "youtube_link": "https://www.youtube.com/watch?v={video_id}",
  "restaurants": [
