...
- 🤖 **자동화된 크롤링**: Perplexity AI를 활용한 지능적인 맛집 정보 추출 (하나의 영상에서 여러 식당 동시 추출, AI 모델 자동 감지)
- 🔐 **지능적 세션 관리**: 로그인 세션 자동 저장/복원으로 장시간 크롤링 지원 (세션 복원 성공 시 바로 크롤링 시작)
- 🔄 **자동 로그인 유지**: 세션 만료 시 자동 재로그인으로 중단 없는 작업
- 🧹 **깔끔한 데이터**: 출처 인용구 및 빈 괄호 자동 제거 ([1], [web:7], {ts:670}, (, , , ) 등)로 완벽하게 깔끔한 JSON 데이터 생성
...
```
**인용구 정리**: 기존 데이터에서 [attached_file:1], [attached-file:1], [web:7], [2], [3], {ts:670}, (, , , ) 등의 모든 출처 인용구와 빈 괄호를 제거합니다.
