...
mod tests {
    ...
    fn gemini_accepts_canonical_category_lists() {
        let data = dict(vec![
            ("youtube_link", s("https://www.youtube.com/watch?v=category-list")),
            (
                "restaurants",
                list(vec![dict(vec![
                    ...
                    ("reasoning_basis", s("영상과 자막에서 방문 위치와 상호가 명확하게 확인됩니다.")),
                    ("youtuber_review", s("여러 메뉴를 직접 먹고 맛과 구성에 관해 구체적으로 평가했습니다.")),
                ])]),
            ),
        ]);
    ...
    fn gemini_keeps_legacy_scalar_category_compatible() {
        let data = dict(vec![
            ("youtube_link", s("https://www.youtube.com/watch?v=legacy-scalar")),
            (
                "restaurants",
                list(vec![dict(vec![
                    ...
                    ("category", s("일식")),
                    ("reasoning_basis", s("영상과 자막에서 방문 위치와 상호가 명확하게 확인됩니다.")),
                    ("youtuber_review", s("메뉴를 직접 먹고 맛과 구성에 관해 구체적으로 평가했습니다.")),
                ])]),
            ),
        ]);
    ...
    fn gemini_rejects_legacy_label_inside_canonical_list() {
        let data = dict(vec![
            ("youtube_link", s("https://www.youtube.com/watch?v=legacy-list")),
            (
                "restaurants",
                list(vec![dict(vec![
        ...
        let invalid: Vec<_> = errors.iter().filter(|e| e.rule == "invalid_category").collect();
    ...
    fn gemini_reports_malformed_category_without_crashing() {
        let data = dict(vec![
            ("youtube_link", s("https://www.youtube.com/watch?v=list-malformed")),
            (
                "restaurants",
                list(vec![dict(vec![
                    ("origin_name", s("분류형식검증식당")),
                    ("address", s("서울특별시 중구 세종대로 1")),
                    ("category", list(vec![s("한식"), dict(vec![("name", s("분식"))])])),
                ])]),
            ),
        ]);
    ...
    fn gemini_rejects_falsy_members_inside_category_list() {
        for item in [s(""), Value::None, Value::Int(0), Value::Bool(false)] {
            let data = dict(vec![
                ("youtube_link", s("https://www.youtube.com/watch?v=list-falsy")),
                (
                    "restaurants",
                    list(vec![dict(vec![
            ...
            let invalid: Vec<_> =
                errors.iter().filter(|e| e.rule == "invalid_category").collect();
    ...
    fn selection_detects_missing_and_extra_names() {
        ...
        let mismatches: Vec<_> = errors.iter().filter(|e| e.rule == "name_mismatch").collect();
        ...
        let data = dict(vec![(
            ...
            dict(vec![
                ...
                (
                    ...
                    list(vec![dict(vec![("name", s("식당A")), ("eval_value", Value::Bool(true))])]),
                ),
            ]),
        )]);
    ...
    fn laaj_detects_score_and_type_regressions() {
        let data = dict(vec![
            ("restaurants", list(vec![dict(vec![("origin_name", s("식당A"))])])),
            (
                "evaluation_results",
                dict(vec![
                    (
                    ...
                    (
                        "rb_grounding_TF",
                        list(vec![dict(vec![("name", s("식당A")), ("eval_value", s("true"))])]),
                    ),
                    (
    ...
    fn laaj_accepts_values_wrappers() {
        let values_wrap = |items: Vec<Value>| dict(vec![("values", list(items)), ("missing", list(vec![]))]);
        let data = dict(vec![
            ("restaurants", list(vec![dict(vec![("origin_name", s("식당A"))])])),
            (
                "evaluation_results",
        ...
        let rule_data = dict(vec![
            ...
            (
                ...
                dict(vec![
                    (
                        "location_match_TF",
                        list(vec![dict(vec![("origin_name", s("식당A")), ("eval_value", Value::Bool(false))])]),
                    ),
                    (
                        "category_validity_TF",
                        list(vec![dict(vec![("name", s("식당A")), ("eval_value", Value::Bool(false))])]),
                    ),
                ]),
            ),
        ]);
        let laaj_data = dict(vec![
            ...
            (
                ...
                dict(vec![
                    (
                        "visit_authenticity",
                        list(vec![dict(vec![("name", s("식당A")), ("eval_value", Value::Int(2))])]),
                    ),
                    (
                        "category_TF",
                        list(vec![dict(vec![("name", s("식당A")), ("eval_value", Value::Bool(true))])]),
    ...
    fn transform_detects_required_fields_and_duplicates() {
        let records = vec![
            ...
            dict(vec![
                ...
                ("lat", Value::Int(10)),
                ("lng", Value::Int(200)),
                ("evaluation_results", dict(vec![("dummy", Value::Bool(true))])),
            ]),
        ];
