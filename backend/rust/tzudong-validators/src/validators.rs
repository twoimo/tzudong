...
const CANONICAL_CATEGORIES: &[&str] = &[
    "치킨", "중식", "돈까스·회", "피자", "패스트푸드", "찜·탕", "족발·보쌈", "분식",
    "카페·디저트", "한식", "고기", "양식", "아시안", "야식", "도시락",
];
const LEGACY_SCALAR_CATEGORIES: &[&str] = &[
    "일식", "카페", "디저트", "해산물", "뷔페", "베이커리", "술집", "기타", "간식",
    "브런치", "샐러드", "샌드위치", "면", "국밥",
];
...
pub fn validate_gemini_output(video_id: &str, data: &Value) -> Vec<ValidationError> {
    ...
    if !data.get("youtube_link").truthy() {
        errors.push(
            ValidationError::new(step, video_id, ERROR, "required_field", "youtube_link 필드 누락".to_string())
    ...
    if !restaurants.is_list() {
        errors.push(
            ValidationError::new(step, video_id, ERROR, "type_error", "restaurants가 리스트가 아닙니다".to_string())
...
pub fn validate_rule_results(video_id: &str, data: &Value) -> Vec<ValidationError> {
    ...
    if !location_matches.is_list() {
        ...
        for (idx, loc) in loc_items.iter().enumerate() {
            ...
            let provider_truthy =
                loc.get("naver_name").truthy() || loc.get("google_name").truthy();
                ...
                if !second_pass.is_dict() {
                    errors.push(
                        ValidationError::new(
                            ...
                            "timeout/rate_limited 상태인데 second_pass 메타데이터가 없음".to_string(),
                        )
                        .with_name(origin_name.clone()),
                    );
                } else if pending_str == Some("timeout")
                    && !second_pass.get("timed_out").is_true()
                {
                    errors.push(
                        ValidationError::new(
                            step,
                            ...
                            "pending_reason=timeout인데 second_pass.timed_out가 true가 아님".to_string(),
                        )
                        .with_name(origin_name.clone()),
                    );
            ...
            if !val.is_none() && !val.is_bool() {
                errors.push(
                    ValidationError::new(
                        ...
                        WARNING,
                        "type_error",
                        format!("{}.eval_value가 boolean이 아닙니다: {}", bool_key, py_str(val)),
                    )
...
pub fn cross_validate(
    ...
    let default_dict = Value::Dict(Vec::new());
    let rule_eval = rule_data.get_opt("evaluation_results").unwrap_or(&default_dict);
    let laaj_eval = laaj_data.get_opt("evaluation_results").unwrap_or(&default_dict);
    ...
    if !name_diff.is_empty() {
        errors.push(ValidationError::new(
            ...
            format!("Rule vs LAAJ 음식점 이름 불일치: {}", join_refs(&name_diff, ", ")),
        ));
    }
...
pub fn validate_transform_output(video_id: &str, records: &[Value]) -> Vec<ValidationError> {
    ...
    for (idx, record) in records.iter().enumerate() {
        ...
        for &field in TRANSFORM_REQUIRED_FIELDS_SORTED {
            ...
            let empty_allowed = (field == "lat" || field == "lng")
                && allows_empty_transform_geocoding(record);
...
fn allows_pending_transform_geocoding(record: &Value) -> bool {
    ...
    let stage_ok = stage.is_none()
        || matches!(stage, Value::Int(1))
        || matches!(stage, Value::Int(2));
