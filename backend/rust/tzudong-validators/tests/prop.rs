...
fn reorder(value: &Value, k: usize) -> Value {
    match value {
        Value::Dict(pairs) => {
            let mut rotated: Vec<(String, Value)> =
                pairs.iter().map(|(key, v)| (key.clone(), reorder(v, k))).collect();
            let len = rotated.len();
            if len != 0 {
...
fn category() -> impl Strategy<Value = Value> {
    ...
    prop_oneof![
        ...
        prop::sample::select(vec!["외계음식", "없는카테고리"]).prop_map(|s| Value::Str(s.to_string())),
        prop::collection::vec(
...
fn valid_gemini_input() -> impl Strategy<Value = Value> {
    ...
    (link, restaurants).prop_map(|(l, r)| {
        Value::Dict(vec![("youtube_link".into(), l), ("restaurants".into(), r)])
    })
}
...
fn valid_rule_input() -> impl Strategy<Value = Value> {
    ...
    let cat = prop::collection::vec(
        (name_str(), any::<bool>())
            .prop_map(|(n, b)| Value::Dict(vec![("name".into(), n), ("eval_value".into(), Value::Bool(b))])),
        0..3,
    )
    .prop_map(Value::List);
    ...
    (
        ...
        .prop_map(|(tid, nm, lat, lng, ev)| {
            Value::Dict(vec![
                ...
                ("youtube_link".into(), Value::Str("https://youtu.be/x".into())),
