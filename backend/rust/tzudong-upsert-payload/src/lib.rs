...
mod tests {
    ...
    fn db_error_classification_matches_python_order() {
        ...
        assert_eq!(map_db_error("batch_upsert_limit exceeded", None), BATCH_UPSERT_LIMIT);
        assert_eq!(map_db_error("batch_upsert_invalid row", None), BATCH_UPSERT_INVALID);
        assert_eq!(
            map_db_error("conditional_write_failed", None),
            CONDITIONAL_WRITE_FAILED
        );
        assert_eq!(map_db_error("deadlock", Some("40001")), COMPARE_AND_SET_CONFLICT);
        assert_eq!(map_db_error("something else", Some("23505")), CONDITIONAL_WRITE_FAILED);
