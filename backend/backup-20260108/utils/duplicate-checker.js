 ...
 */
export function loadProcessedUrls(filePath) {
    const urls = new Set();
 ...
 */
export function loadProcessedRestaurants(filePath, key = 'name', nestedKey = 'restaurants') {
 ...
 * }
 */
export function loadProcessedUniqueIds(filePath) {
    const ids = new Set();
    if (!fs.existsSync(filePath)) {
        return ids;
    }
 ...
 * );
 */
export function filterNewItems(inputItems, processedItems, keyFunc) {
 ...
 */
export function appendToJsonl(filePath, data, createDirs = true) {
    let count = 0;
    // 디렉토리가 없으면 생성
    if (createDirs) {
 ...
 * );
 */
export function loadMultipleProcessedUrls(...filePaths) {
 ...
 * );
 */
export function loadMultipleProcessedRestaurants(filePaths, key = 'name', nestedKey = 'restaurants') {
