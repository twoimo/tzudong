...
/**
 * JSONL 파일에서 처리된 youtube_link 추출 (중복 검사용)
 */
function loadProcessedUrls(filePath: string): Set<string> {
  ...
  try {
    ...
    console.error(`⚠️  파일 읽기 실패 (${filePath}):`, err);
  }
...
async function processRemaining() {
  ...
  try {
    ...
    console.log(`✅ 이미 처리된 URL: ${processedUrls.size}개\n`);

    // 2. JSONL 파일에서 null 값이 있는 항목들 찾기
    const processor = new JsonlProcessor('./tzuyang_restaurant_results.jsonl');
    const entries = processor.readAllEntries();
    
    // 3. null 항목 중 이미 처리된 URL 제외
    const nullEntries = entries.filter(entry => 
      (!entry.restaurants || entry.restaurants.length === 0) &&
      !processedUrls.has(entry.youtube_link)
    );
    
    const totalNullCount = entries.filter(entry => !entry.restaurants || entry.restaurants.length === 0).length;
    const remainingCount = nullEntries.length;
    const duplicateCount = totalNullCount - remainingCount;
    ...
    console.log(`📋 처리 대상:`);
    console.log(`   - 전체 null 항목: ${totalNullCount}개`);
    console.log(`   - 중복 제외: ${duplicateCount}개`);
    console.log(`   - 새로 처리할 항목: ${remainingCount}개\n`);

    // 테스트 모드에서는 1개만 처리
    const TEST_MODE = process.env.TEST_MODE === 'true';
    const maxToProcess = TEST_MODE ? 1 : Math.min(10, remainingCount); // 최대 10개로 제한

    console.log(`🎯 이번 배치 처리: ${maxToProcess}개 (전체 남은 항목: ${remainingCount}개)`);
