...
- (매우 중요) 유튜브 영상 내의 **식당 간판, 영상 내 자막, 메뉴판 등의 시각 정보**를 반드시 최우선으로 정밀 분석하여, 식당명과 **'특정 지점명'(예: 약수점)**까지 반드시 확보합니다. 음성 기록(transcript)이나 제목(title)은 보조 자료로만 활용하며, 시각 정보가 음성 기록과 다를 경우 **시각 정보를 최우선으로 신뢰합니다.**
...
- tzuyang_review: 4단계에서 요약한 쯔양의 리뷰 내용 (없으면 null).
...
2) 출력 시 key는 name, phone, address, lat, lng, category, youtube_link, reasoning_basis, tzuyang_review만 있어야 함.
