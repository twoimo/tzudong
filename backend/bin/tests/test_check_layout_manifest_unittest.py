...
correspondence, the deploy/pipeline_control ownership split, proposed-move
residual counting, alias/symlink rejection, and stale-reference scanning — plus
one end-to-end assertion that the current candidate manifest passes against the
real tree without inventing a directory move. Following the
...
PROPOSED_MOVE = (("backend/pipeline-control", "backend/deploy/pipeline-control"),)
...
class DirectoryMoveTest(unittest.TestCase):
    def test_no_directory_move_is_approved_by_default(self):
        self.assertEqual(clm.DEFAULT_MOVES, ())
...
class AliasPathTest(unittest.TestCase):
    def test_clean_tree_passes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = clm.check_alias_paths(root, moves=PROPOSED_MOVE)
            self.assertTrue(result["ok"], result)
...
class StaleReferenceTest(unittest.TestCase):
    def test_only_exact_declared_ephemeral_directory_may_be_absent(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            compose = root / "backend/pipeline-control/docker-compose.observability.yml"
            compose.parent.mkdir(parents=True)
            compose.write_text("volumes:\n  - ../log:/var/log/tzudong:ro\n", encoding="utf-8")
            self.assertFalse(clm.scan_stale_references(root)["ok"])
            result = clm.scan_stale_references(root, local_ephemeral_directories=("backend/log",))
            self.assertTrue(result["ok"], result)
            self.assertFalse((root / "backend/log").exists())

            for source in ("../log/config.yaml", "../logs", "../missing.yaml"):
                with self.subTest(source=source):
                    compose.write_text(f"volumes:\n  - {source}:/mount:ro\n", encoding="utf-8")
                    result = clm.scan_stale_references(root, local_ephemeral_directories=("backend/log",))
                    self.assertFalse(result["ok"], result)

    def test_ephemeral_exception_cannot_escape_or_follow_an_external_symlink(self):
        with tempfile.TemporaryDirectory() as tmp:
            parent = Path(tmp)
            root = parent / "repo"
            compose = root / "backend/pipeline-control/docker-compose.observability.yml"
            ...
            external.mkdir()
            (root / "backend/log").symlink_to(external, target_is_directory=True)
            for source, declared in (
                ...
                with self.subTest(source=source, declared=declared):
                    compose.write_text(f"volumes:\n  - {source}:/mount:ro\n", encoding="utf-8")
                    result = clm.scan_stale_references(root, local_ephemeral_directories=(declared,))
                    self.assertFalse(result["ok"], result)

    def test_cyclic_runtime_symlink_returns_a_bounded_failure(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            compose = root / "backend/pipeline-control/docker-compose.observability.yml"
            compose.parent.mkdir(parents=True)
            (root / "backend/log").symlink_to("log", target_is_directory=True)
            compose.write_text("volumes:\n  - ../log:/var/log/tzudong:ro\n", encoding="utf-8")
            result = clm.scan_stale_references(root, local_ephemeral_directories=("backend/log",))
            self.assertFalse(result["ok"])
    ...
    def test_current_operational_path_is_not_stale_by_default(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
    ...
    def test_clean_tree_reports_zero(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = clm.scan_stale_references(root, moves=PROPOSED_MOVE)
            self.assertTrue(result["ok"], result)
...
class RealTreeTest(unittest.TestCase):
    def test_committed_manifest_passes_on_real_tree(self):
        # End-to-end: the candidate manifest + current candidate tree pass
        # while retaining the approved hyphenated operational-asset path.
        ...
        self.assertIn("backend/pipeline-control", paths)
        self.assertIn("backend/pipeline_control", paths)
        self.assertNotIn("backend/deploy/pipeline-control", paths)
