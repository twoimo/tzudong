#!/usr/bin/env python3
"""Unit tests for the P2 Phase_Gate runner ``backend/bin/run_p2_gate.py`` (Task 18).
...
_RUNNER_PATH = _ROOT / "backend" / "bin" / "run_p2_gate.py"
...
run_p2_gate = _load("run_p2_gate", _RUNNER_PATH)
...
def _additional_runner(*, env_ok: bool, pbt_ok: bool):
    """Return a fake additional-command runner for the two P2 verifications."""
    ...
    def runner(cwd, argv):
        ...
        if "unittest" in argv and any("test_publish_" in a for a in argv):
...
class RunP2GateTest(unittest.TestCase):
    def test_secrets_absent_fails_closed_and_web_commands_not_executed(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = json.loads(
                (Path(tmp) / "P2-publication-report.json").read_text("utf-8")
            )
            # Well-formed D9 core.
            self.assertEqual(report["schemaVersion"], 1)
            self.assertEqual(report["phaseId"], "P2-publication")
            self.assertEqual(report["sequence"], 2)
            self.assertEqual(report["assignedRequirements"], [10])
            self.assertEqual(report["rollbackPlanRef"], run_p2_gate.P2_ROLLBACK_PLAN_REF)
            self.assertIsInstance(report["unexplainedWorktreeChanges"], list)
            # Additional P2 verifications recorded, env-contract failed closed.
            labels = {r["label"]: r for r in report["additionalVerificationCommands"]}
            self.assertIn("check_env_contract_daily", labels)
            self.assertIn("publish_p2_pbt", labels)
            self.assertFalse(labels["check_env_contract_daily"]["passed"])
            self.assertTrue(labels["publish_p2_pbt"]["passed"])
    ...
    def test_all_pass_yields_satisfied_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            # Seed the P2 execution-evidence artifact so X1/X2 hold.
            root = Path(tmp) / "repo"
            (root / "backend" / "log" / "phases").mkdir(parents=True)
            (root / "backend" / "log" / "phases" / "P2-publication-execution-summary.json").write_text(
                json.dumps({"allStagesPassed": True, "idempotentConvergence": True}),
                "utf-8",
            )
            ...
            self.assertTrue(by_id["P2-X1"]["satisfied"])
            self.assertTrue(by_id["P2-X2"]["satisfied"])
            self.assertTrue(by_id["P2-X3"]["satisfied"])
            self.assertTrue(by_id["P2-X4"]["satisfied"])

    def test_five_stages_unmet_yields_not_satisfied(self):
        with tempfile.TemporaryDirectory() as tmp:
            # Seed an execution summary where the five stages did not all pass.
            root = Path(tmp) / "repo"
            (root / "backend" / "log" / "phases").mkdir(parents=True)
            (root / "backend" / "log" / "phases" / "P2-publication-execution-summary.json").write_text(
                json.dumps({"allStagesPassed": False, "idempotentConvergence": False}),
                "utf-8",
            )
            ...
            self.assertIn("P2-X1", result["unsatisfiedConditionIds"])
            self.assertIn("P2-X2", result["unsatisfiedConditionIds"])

    def test_referenced_rollback_plan_is_valid(self):
        plan = json.loads(
            (_ROOT / run_p2_gate.P2_ROLLBACK_PLAN_REF).read_text("utf-8")
        )
