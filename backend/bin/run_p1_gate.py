#!/usr/bin/env python3
"""P1 (P1-local-pipeline) Phase_Gate verification runner (Task 10).

This wires the cross-cutting Phase_Gate runner (``backend/bin/phase_gate.py``,
Task 1 / design C12) to the concrete P1 phase from design's "단계 순서와
Phase_Gate" table (sequence 1, requirements 8 and 9). It:
  ...
  * Writes the D9 report to ``backend/log/phases/P1-local-pipeline-report.json``.

Completion condition (design table row P1): Hosted write count 0,
Schema_Mirror_Report defect count 0, all verification commands pass, and every
enumerated public route responds within 5s without a server error.

Environment reality honoured honestly (no fabrication):
  ...
  * The property-based suite needs ``hypothesis``. The repo's system
    ``python3`` lacks it; the project ``.venv/bin/python`` has it. The PBT
    discover command is therefore run with the venv interpreter when present,
    and the interpreter's repo-relative path is recorded.
  * When the operator-secret precondition fails closed, P1's execution
    completion evidence (Hosted-write count from a real ``heavy_local``+
    ``local_db`` run, and a hosted-read Schema_Mirror_Report) cannot be
...
def _repo_root() -> Path:
    # backend/bin/run_p1_gate.py -> backend/bin -> backend -> <repo root>
...
# ---------------------------------------------------------------------------
# P1 configuration (design "단계 순서와 Phase_Gate", row 1; D9 example).
# ---------------------------------------------------------------------------
...
P1_ENTRY_CONDITIONS: tuple[dict, ...] = (
    {
        "conditionId": "P1-E1",
        "statement": "로컬 14서비스 Supabase 스택이 전부 준비 상태다",
        "satisfied": None,
    },
)

# Exit (completion) conditions from the design table for P1.
_EXIT_HOSTED_WRITES = "P1-X1"
_EXIT_SCHEMA_MIRROR = "P1-X2"
_EXIT_COMMANDS = "P1-X3"
_EXIT_ROUTES = "P1-X4"

P1_EXIT_STATEMENTS: dict[str, str] = {
    _EXIT_HOSTED_WRITES: "heavy_local+local_db 실행에서 Hosted 쓰기 요청 수가 0이다",
    _EXIT_SCHEMA_MIRROR: "Schema_Mirror_Report 결함 건수가 0이다",
    _EXIT_COMMANDS: "7개 검증 명령 전부 성공이다",
    _EXIT_ROUTES: "열거된 공개 라우트 전부가 5초 이내 서버 오류 없이 응답했다",
}

# Web_App public (non-admin) routes enumerated for the P1 gate's route check
# (Requirement 16.10). Bounded route strings only.
P1_PUBLIC_ROUTES: tuple[str, ...] = (
    "/",
    "/global-map",
    "/insights",
    "/leaderboard",
...
# Artifacts that carry P1 execution completion evidence, if a real
# heavy_local+local_db run and a Schema_Mirror_Report have been produced.
_EXECUTION_SUMMARY_ARTIFACT = "backend/log/phases/P1-local-pipeline-execution-summary.json"
_SCHEMA_MIRROR_ARTIFACT = "backend/log/schema-mirror-report.json"

# Fixed, bounded note recorded against a verification command that was not
# executed because a P1 precondition failed closed. Never a diagnostic string.
...
# ---------------------------------------------------------------------------
# Additional P1 verification commands (Task 10): env-contract + PBT discover.
# ---------------------------------------------------------------------------
...
def run_additional_verifications(
    *,
    ...
    """Run the two P1-specific additional verifications; return bounded records.
    ...
    names, values, or any Forbidden_Log_Field. The PBT discover command is run
    with the venv interpreter (hypothesis) when available.
    """
    ...
    # 2) pipeline_control property-based suite (needs hypothesis => venv python).
    pbt_interp = _venv_python(repo_root)
    # ``-t .`` pins the top-level dir to the repo root so unittest discovery
    # does NOT place ``backend/pipeline_control`` on sys.path[0]; without it the
    # package's local ``queue.py`` shadows the stdlib ``queue`` that hypothesis
    # imports and every test errors on collection. This is how the package's
    # suite is meant to run, not a weakening of the check.
    pbt_argv = (
        pbt_interp,
        "-m",
        "unittest",
    ...
    records.append(
        {
            "label": "pipeline_control_pbt_discover",
            "cwd": ".",
            "command": (
                'python -m unittest discover -t . -s backend/pipeline_control '
                '-p "test_*_pbt.py"'
            ),
...
def _hosted_writes_satisfied(repo_root: Path) -> bool | None:
    """X1: Hosted write count is 0 in the heavy_local+local_db run.

    True only when an execution-summary artifact records
    ``hostedWriteRequestCount == 0``. Absent artifact -> ``None`` (unverified);
    a positive count -> ``False``.
    """

    summary = _read_json_artifact(repo_root, _EXECUTION_SUMMARY_ARTIFACT)
    if summary is None:
        return None
    count = summary.get("hostedWriteRequestCount")
    if isinstance(count, bool) or not isinstance(count, int):
        return None
    return count == 0


def _schema_mirror_satisfied(repo_root: Path) -> bool | None:
    """X2: Schema_Mirror_Report defect count is 0.

    True only when the report artifact is complete and passes. Absent artifact
    -> ``None`` (unverified).
    """

    report = _read_json_artifact(repo_root, _SCHEMA_MIRROR_ARTIFACT)
    if report is None:
        return None
    complete = report.get("complete")
    mirror_pass = report.get("mirrorPass")
    if complete is True and mirror_pass is True:
        return True
    return False
...
def build_p1_report(
    *,
    ...
    """Assemble the P1 phase-report input for ``evaluate_phase_gate``.
    ...
    exit_conditions = [
        {
            "conditionId": cid,
            "statement": P1_EXIT_STATEMENTS[cid],
            "satisfied": overrides.get(cid, None),
        }
        for cid in (_EXIT_HOSTED_WRITES, _EXIT_SCHEMA_MIRROR, _EXIT_COMMANDS, _EXIT_ROUTES)
    ]

    return {
        "phaseId": P1_PHASE_ID,
        "sequence": P1_SEQUENCE,
        "assignedRequirements": list(P1_ASSIGNED_REQUIREMENTS),
        "worktreeId": worktree_id or _worktree_id(repo_root),
        "entryConditions": [dict(cond) for cond in P1_ENTRY_CONDITIONS],
        "exitConditions": exit_conditions,
        "publicRouteChecks": list(P1_PUBLIC_ROUTES),
        "unexplainedWorktreeChanges": list(unexplained_worktree_changes or []),
        "rollbackPlanRef": P1_ROLLBACK_PLAN_REF,
    }
...
def _make_command_runner(
    *,
    ...
    worktree, and without operator secrets P1 completion cannot be established
...
def run_p1_gate(
    *,
    ...
    """Run the P1 Phase_Gate and write the D9 report.
    ...
    exit_overrides: dict[str, bool | None] = {
        _EXIT_HOSTED_WRITES: _hosted_writes_satisfied(root),
        _EXIT_SCHEMA_MIRROR: _schema_mirror_satisfied(root),
    ...
    report_input = build_p1_report(repo_root=root, exit_overrides=exit_overrides)
    ...
    # Merge the two P1-specific additional verifications and a bounded
...
def main(argv: Sequence[str] | None = None) -> int:
    """CLI: run the P1 gate and print a bounded summary; set the exit code.
    ...
    parser = argparse.ArgumentParser(
        description="Run the P1-local-pipeline Phase_Gate and write its D9 report"
    )
    parser.add_argument(
        "--run-web-commands",
        action="store_true",
    ...
    result = run_p1_gate(run_web_commands=args.run_web_commands)

    if args.json:
        print(json.dumps(result, ensure_ascii=True, sort_keys=True))
    else:
        code = result["resultCode"] or "pass"
        print(f"P1 gate result: {code}")
