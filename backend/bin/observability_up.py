...
under ``backend/pipeline-control/`` (the current repository-owned path) —
...
# The three required services readiness is individually checked for (12.1):
...
_COMPOSE_DIR = _ROOT / "backend" / "pipeline-control"
...
def start_observability_stack(
    *,
    ...
    if not enable_loki:
        artifact["notStartedComponents"].append(
            {"component": "loki", "reasonCode": OPTIONAL_COMPONENT_NOT_STARTED}
        )
    ...
    for overlay in overlays:
        ...
        if overlay == "observability" and not enable_loki:
            argv += ("otel-collector", "prometheus", "grafana")
        ...
        if not isinstance(run, Mapping) or run.get("exitCode") != 0:
            ...
            artifact["errorCode"] = SERVICE_READINESS_TIMEOUT
            readiness_services = CORE_SERVICES + (("loki",) if enable_loki else ())
            artifact["notReady"] = sorted(readiness_services)
            artifact["services"] = _service_rows(
                {
                    service: {"readyState": "not_ready", "elapsedSeconds": 0.0}
                    for service in readiness_services
                },
                images,
                services=readiness_services,
            )
            return artifact

    # 6. Individually re-check readiness for all required services (12.1).
    readiness_services = CORE_SERVICES + (("loki",) if enable_loki else ())
    ...
    artifact["services"] = _service_rows(
        readiness["readiness"], images, services=readiness_services
    )
...
def _service_rows(
    ...
    *,
    services: Sequence[str] = CORE_SERVICES,
) -> list[dict]:
...
def _http_readiness_probe(service: str) -> bool:
    ...
    import urllib.error  # local import: keep module import side-effect free
    import urllib.request  # local import: keep module import side-effect free

    endpoints = {
        # The OTLP/HTTP receiver deliberately has no generic health route. A GET
        # to its exact metrics ingestion route returns 405 when the listener is
        # ready; only that service/status pair is admitted below.
        "otel-collector": "http://127.0.0.1:4318/v1/metrics",
