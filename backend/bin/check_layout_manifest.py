...
git-tracked tree and enforces the directory-ownership reorganization landed by
task 34 (``backend/pipeline-control`` -> ``backend/deploy/pipeline-control``).
...
# The completed reorganization move(s). Stale-reference scanning ties broken
# references to these moved-from tokens (Requirement 6.7).
DEFAULT_MOVES: tuple[tuple[str, str], ...] = (
    ("backend/pipeline-control", "backend/deploy/pipeline-control"),
)
...
_COMPOSE_SCAN_DIR = "backend/deploy"
...
def _collect_scan_targets(root: Path) -> list[Path]:
    ...
    compose_dir = root / _COMPOSE_SCAN_DIR
...
def scan_stale_references(
    ...
    *,
    ...
    Three reference classes are resolved:
      ...
      * moved-tree compose relative volume sources (``./`` / ``../``) that do
        not resolve relative to the compose file.
...
def run_check(
    *,
    ...
    stale = scan_stale_references(root_path, moves=moves)
