...
| Prometheus / OTel pipeline metrics | `backend/pipeline_control/metrics.py` + `backend/pipeline-control/metrics.v1.json` | operator Prometheus/Grafana overlay | frozen names `tzudong_pipeline_runs_enqueued_total`, `tzudong_pipeline_runs_claimed_total`, `tzudong_pipeline_runs_succeeded_total`, `tzudong_pipeline_runs_failed_total`; MemoryStore enqueue/claim/Succeeded/Failed call record() as noop (idempotency replay and Cancelled/Paused finish_dry_run do not); record() exports via SDK when overlay OTEL endpoint is set; image is no longer copy-only-only-kafka-python; GHA export attempt fail-closed; overlay `docker-compose.observability.yml` scrapes `otel-collector:8889` and binds Grafana at `127.0.0.1:3001` as an operator browser. Kafka lag / ES rows/sec remain deferred. Admin iframe stays `forbidden_until_csp_auth_gate`. Do not add `TZUDONG_PIPELINE_METRICS` to overlay or GHA. | `backend.pipeline_control.tests.test_metrics` |
