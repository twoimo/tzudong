...
const HEATMAPS_DIR = path.join(DATA_DIR, 'heatmapMarkers');
...
export async function collectHeatmap(browser, videoId) {
  ...
  try {
    ...
    const heatmapData = await page.evaluate(() => {
      ...
      return {
        hasHeatmap: heatMarkers && heatMarkers.length > 0,
        // 메타데이터
        meta: {
          title,
    ...
    // heatmapMarkers가 없으면 실패
    if (!heatmapData.heatmapMarkers || heatmapData.heatmapMarkers.length === 0) {
      console.log(`  ⚠️ 히트맵 마커 데이터 없음`);
      return null;
    }
