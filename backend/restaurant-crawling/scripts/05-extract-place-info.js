 ...
 * 
 * 사용법:
 *   node 05-extract-place-info.js --channel tzuyang
 *   node 05-extract-place-info.js --channel meatcreator
 *   node 05-extract-place-info.js  # 모든 채널
 */
...
const envPaths = [
    ...
    path.resolve(__dirname, '../.env'),      // restaurant-crawling/.env
];
...
/**
 * 채널 단일 처리 - meta에서 mapUrls 추출하여 place_info 수집
 */
async function collectChannelPlaceInfo(channelName, channelConfig) {
    const dataPath = path.resolve(__dirname, '../../', channelConfig.data_path);
    const metaDir = path.join(dataPath, 'meta');
    const placeInfoDir = path.join(dataPath, 'place_info');
    
    // 폴더 생성
    if (!fs.existsSync(placeInfoDir)) {
        fs.mkdirSync(placeInfoDir, { recursive: true });
    }
    
    // meta 파일에서 mapUrls 포함 영상 목록 추출
    if (!fs.existsSync(metaDir)) {
        log('warning', `Meta 폴더 없음: ${metaDir}`);
        return { channel: channelName, processed: 0, success: 0 };
    }
    
    const metaFiles = fs.readdirSync(metaDir).filter(f => f.endsWith('.jsonl'));
    
    // 이미 처리된 place_info 확인
    const existingIds = new Set(
        fs.existsSync(placeInfoDir)
            ? fs.readdirSync(placeInfoDir).filter(f => f.endsWith('.jsonl')).map(f => f.replace('.jsonl', ''))
            : []
    );
    
    // mapUrls가 있는 영상만 선별
    const videosWithMapUrls = [];
    for (const file of metaFiles) {
        const videoId = file.replace('.jsonl', '');
        if (existingIds.has(videoId)) continue; // 이미 처리됨
        
        try {
            const content = fs.readFileSync(path.join(metaDir, file), 'utf-8').trim();
            if (!content) continue;
            const meta = JSON.parse(content.split('\n')[0]);
            
            // description에서 지도 URL 추출
            const mapUrls = extractMapUrlsFromDescription(meta.description || '');
            if (mapUrls.length > 0) {
                videosWithMapUrls.push({
                    videoId,
                    title: meta.title,
                    mapUrls
                });
            ...
            log('debug', `Meta 파싱 실패: ${file} - ${e.message}`);
        }
    }
    
    log('info', `채널: ${channelConfig.name}`);
    log('info', `  전체 Meta: ${metaFiles.length}개, 지도URL 포함: ${videosWithMapUrls.length}개`);
    
    if (videosWithMapUrls.length === 0) {
        log('success', '  수집 대상 없음');
        return { channel: channelName, processed: 0, success: 0, totalPlaces: 0 };
    }
    
    const stats = { success: 0, noPlaceInfo: 0, totalPlaces: 0, failed: 0 };
    
    for (let i = 0; i < videosWithMapUrls.length; i++) {
        const video = videosWithMapUrls[i];
        log('info', `  [${i + 1}/${videosWithMapUrls.length}] ${video.title?.slice(0, 40) || video.videoId}... (${video.mapUrls.length}개 URL)`);
        
        const result = await collectPlaceInfoForVideo(video, i, videosWithMapUrls.length);
        
        // place_info 저장 (video_id 단위)
        const outputFile = path.join(placeInfoDir, `${video.videoId}.jsonl`);
        fs.writeFileSync(outputFile, JSON.stringify(result) + '\n', 'utf-8');
        
        if (result.hasPlaceInfo) {
            stats.success++;
            stats.totalPlaces += result.places.length;
            log('success', `    → ${result.places.length}개 장소 수집`);
        } else {
            stats.noPlaceInfo++;
            log('debug', `    → 장소 정보 없음`);
        }
    ...
    return {
        channel: channelName,
        processed: videosWithMapUrls.length,
        success: stats.success,
        noPlaceInfo: stats.noPlaceInfo,
 ...
 * description에서 지도 URL 추출
 */
function extractMapUrlsFromDescription(description) {
    ...
    const naverPatterns = [
        /https?:\/\/naver\.me\/[^\s]+/g,
        /https?:\/\/(?:map|place)\.naver\.com\/[^\s]+/g,
        /https?:\/\/pcmap\.place\.naver\.com\/[^\s]+/g
    ];
    
    // 카카오 지도 URL
    const kakaoPatterns = [
        /https?:\/\/kko\.to\/[^\s]+/g,
        /https?:\/\/(?:place\.)?map\.kakao\.com\/[^\s]+/g
    ];
    
    // 구글 지도 URL
    const googlePatterns = [
        /https?:\/\/(?:goo\.gl|maps\.app\.goo\.gl)\/[^\s]+/g,
...
async function main() {
    ...
    for (const result of results) {
        log('info', `  ${result.channel}: 성공 ${result.success}개, 장소 ${result.totalPlaces}개, 정보없음 ${result.noPlaceInfo}개`);
    }
...
export { collectPlaceInfoForVideo, collectChannelPlaceInfo, getBrowser, initPuppeteer };
