...
def get_restaurants_by_video_id(supabase: Client) -> dict[str, list[str]]:
    ...
    """
    print("op=supabase_restaurant_query_started")
    ...
    rows = result.data
    print(f"op=supabase_restaurant_query_complete rows={len(rows)}")
    ...
    print(f"op=restaurants_mapped videos={len(video_restaurants)}")
...
def load_heatmap_by_recollect_id(heatmap_path: Path) -> dict[int, dict]:
    ...
    try:
        ...
        print(f"op=heatmap_load_failed error={safe_error_name(error)}")
        return {}
...
def process_documents(
    ...
    print("op=transcript_metadata_processing_started")

    output_dir.mkdir(parents=True, exist_ok=True)

    if not input_dir.exists():
        print("op=transcript_context_directory_missing")
        return {}

    input_files = list(input_dir.glob("*.jsonl"))
    print(f"op=transcript_metadata_input_files count={len(input_files)}")
    ...
    for input_file in tqdm(input_files, desc="문서 처리"):
        ...
        try:
            ...
            print(f"op=transcript_document_read_failed error={safe_error_name(error)}")
            continue
        ...
        if new_docs_to_write:
            try:
                ...
                print(f"op=transcript_document_write_failed error={safe_error_name(error)}")
        ...
        with open(file_path, "r", encoding="utf-8") as file:
            ...
            if line:
                ...
                if isinstance(docs, list) and len(docs) > 0:
                    ...
                    if "restaurants" in metadata and "is_peak" in metadata:
                        verified_documents += 1

    print(
        f"op=transcript_metadata_verification_complete samples={len(sample_files)} "
...
def main():
    ...
    print("op=transcript_metadata_run_started")
    ...
    try:
        ...
        return
    print("op=supabase_connection_succeeded")
    ...
    print("op=transcript_metadata_paths_resolved")

    # 4. 문서 처리
    stats = process_documents(input_dir, output_dir, heatmap_dir, video_restaurants)

    print(
        f"op=transcript_metadata_processing_complete "
        f"files={stats.get('total_files', 0)} processed={stats.get('processed_files', 0)} "
        f"skipped={stats.get('skipped_files', 0)} documents={stats.get('total_docs', 0)} "
        f"documents_with_restaurants={stats.get('docs_with_restaurants', 0)} "
        f"peak_documents={stats.get('peak_docs', 0)} "
        f"non_peak_documents={stats.get('non_peak_docs', 0)}"
    )

    # 6. 검증
    verify_output(output_dir)

    print("op=transcript_metadata_run_complete")
...
if __name__ == "__main__":
    try:
        ...
        print(f"op=transcript_metadata_failed error={safe_error_name(error)}", file=sys.stderr)
        sys.exit(1)
