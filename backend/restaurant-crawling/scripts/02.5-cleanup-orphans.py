...
def main() -> None:
    ...
    print(f"[SCAN] Scanning {len(transcript_files)} transcript files in '{args.channel}' (Deep Validation)...", file=sys.stderr)
    ...
    if not orphans:
        print("[OK] No orphan files found.", file=sys.stderr)
        return

    print(f"[WARN] Found {len(orphans)} orphan/mismatched files.", file=sys.stderr)
    ...
    for path in orphans:
        try:
            ...
            print(f"  - [ERROR] Failed to delete {filename}: {e}", file=sys.stderr)
            failed_count += 1

    print(f"\n[OK] Cleanup Complete: Deleted {deleted_count} files, Failed {failed_count}.", file=sys.stderr)
