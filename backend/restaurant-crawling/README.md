...
데이터 수집은 **영상의 수명주기(Age)** 와 **변화(Change)** 에 따라 결정됩니다. (바이럴 감지 로직 제거됨)

| 영상 연령 (D+일) | 수집 주기 | 목적 | 비고 |
| :--- | :--- | :--- | :--- |
| **D+0 ~ D+5** | 수집 안 함 | 조회수 안정화 대기 | 단, 최초 1회는 강제 수집 |
| **D+5 ~ D+30** | **주 1회** (Weekly) | 초기 반응 확인 | 생존 신고 |
| **D+30 ~ D+60** | **월 1회** (Monthly) | 중장기 데이터 확보 | 최종 후보군 |
| **D+60 이후** | **중단** (None) | 아카이빙 완료 | 변경 감지 시에만 수집 |

**수집 트리거 (Trigger):**
위 주기와 별개로, 다음 변경사항이 감지되면 **즉시 수집**합니다.
1.  **제목(Title) 변경**
2.  **썸네일(Thumbnail) 변경**
3.  **영상 길이(Duration) 변경**
