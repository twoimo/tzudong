...
def publish_hash_cases(draw: st.DrawFn) -> dict[str, Any]:
    ...
    def draw_published_subset() -> list[str]:
        if not published:
            return []
        return draw(
            st.lists(st.sampled_from(published), unique=True, max_size=len(published))
        )

    def make_valid_row() -> dict[str, Any]:
        # Identity keys are always present so every row is an identifiable,
        # enumerated target; identity uniqueness is irrelevant to the hash.
        row: dict[str, Any] = {
            key: draw(st.integers(min_value=1, max_value=10_000)) for key in identity_keys
        }
    ...
    rows = [make_valid_row() for _ in range(n_rows)]
    ...
    mutate_key = draw(st.sampled_from(sorted(target_row.keys())))
