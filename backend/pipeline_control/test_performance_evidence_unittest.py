...
``backend/pipeline_control/performance_evidence.py``:
  ...
  * canonical evidence path confinement — raw measurements, scorer/validator
    outputs, artifact maps, and canonical inputs remain under
    ``apps/web/performance/*``; any legacy or outside path returns
...
def _valid_evidence_set(**overrides) -> dict:
    base = {
        ...
        "rawArtifactPaths": ["apps/web/performance/raw/R1-validators/run-1.ndjson"],
        "canonicalBudgetInputRef": pe.CANONICAL_BUDGET_INPUT_REF,
        "artifactMapSha256": "d" * 64,
    }
...
class PathSeparationTests(unittest.TestCase):
    ...
    def test_raw_artifact_under_web_tree_passes(self):
        result = pe.check_evidence_path_separation(
            ["apps/web/performance/raw/x.ndjson"], []
        )
        self.assertTrue(result["ok"])
    ...
    def test_parent_traversal_and_sibling_prefix_violate(self):
        for path in (
            "apps/web/performance/../secret.json",
            "apps/web/performance-other/raw.json",
            "apps/web/performance//raw.json",
        ):
            with self.subTest(path=path):
                result = pe.check_evidence_path_separation([path], [])
                self.assertEqual(result["code"], pe.CODE_PATH_VIOLATION)
...
class EvidenceSetStructureTests(unittest.TestCase):
    ...
    def test_budget_drift_and_unknown_metric_are_rejected(self):
        self.assertFalse(
            pe.validate_evidence_set_structure(
                _valid_evidence_set(
                    absoluteBudget={"value": 3_599_999, "unit": "ms"}
                )
            )["ok"]
        )
        self.assertFalse(
            pe.validate_evidence_set_structure(
                _valid_evidence_set(metricKey="backend.unknown")
            )["ok"]
        )

    def test_missing_raw_path_wrong_budget_ref_and_short_map_hash_are_rejected(self):
        cases = (
            {"rawArtifactPaths": []},
            {"canonicalBudgetInputRef": "apps/web/performance/other.json"},
...
class ArtifactMapTests(unittest.TestCase):
    ...
    def test_blank_recorded_hash_fails_closed(self):
        ...
        result = pe.check_artifact_map(
            s, artifact_resolver=lambda _s: {"retrieved": True, "artifactMapSha256": "x" * 64}
        )
        self.assertFalse(result["ok"])
    ...
    def test_not_retrieved_rejected(self):
        result = pe.check_artifact_map(
            _valid_evidence_set(),
            artifact_resolver=lambda _s: {"retrieved": False, "artifactMapSha256": "d" * 64},
        )
        self.assertFalse(result["ok"])
    ...
    def test_hash_mismatch_rejected(self):
        result = pe.check_artifact_map(
            _valid_evidence_set(),
            artifact_resolver=lambda _s: {"retrieved": True, "artifactMapSha256": "e" * 64},
        )
        self.assertFalse(result["ok"])
    ...
    def test_retrieved_and_matching_passes(self):
        result = pe.check_artifact_map(
            _valid_evidence_set(),
            artifact_resolver=lambda _s: {"retrieved": True, "artifactMapSha256": "d" * 64},
        )
        self.assertTrue(result["ok"])
...
class EstablishClaimTests(unittest.TestCase):
    def _resolver_ok(self, _s):
        return {"retrieved": True, "artifactMapSha256": "d" * 64}
    ...
    def test_path_violation_reported_with_path_code(self):
        ...
        bad = _valid_evidence_set(
            rawArtifactPaths=["backend/performance/raw/legacy.ndjson"]
        )
...
class MergeCandidateGateTests(unittest.TestCase):
    def test_all_established_passes(self):
        claims = [
            ...
            {"evidenceSetId": "b", "established": True},
        ]
    ...
    def test_missing_established_flag_is_rejected(self):
        result = pe.check_merge_candidate_claims([{"evidenceSetId": "a"}])
        self.assertFalse(result["ok"])
...
class CanonicalBudgetParityTests(unittest.TestCase):
    ...
    def test_backend_budgets_match_canonical_file(self):
        ...
        for metric_key, expected in pe.BACKEND_METRIC_BUDGETS.items():
            ...
            self.assertEqual(
                expected["relativeBudget"]["thresholdBasisPoints"],
                canonical["relativeThresholdBasisPoints"],
                f"relative budget mismatch for {metric_key}",
            )
            self.assertEqual(
