...
class OtelCollectorWiringTests(unittest.TestCase):
    ...
    def test_native_otlp_exporter_targets_approved_local_host(self) -> None:
        self.assertIn("endpoint: http://loki:3100/otlp", self.text)
        self.assertNotIn("/loki/api/v1/push", self.text)
        self.assertNotIn("/otlp/v1/logs", self.text)
