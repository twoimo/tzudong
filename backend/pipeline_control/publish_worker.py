...
def load_publication_set(path: str | None = None) -> PublicationSet:
    ...
    published columns) that a publish row may carry. It performs no approval
    check — approval gating for the apply phase is Task 14 — it only makes the
    enumerated targets available to the preview validator.
    """
...
def _publication_set_from_document(doc: Any) -> PublicationSet:
    ...
    for entry in raw_tables:
        ...
        if not isinstance(published, Sequence) or isinstance(published, (str, bytes)):
            raise PublishWorkerError("publication_set_shape_invalid")
        identity_keys = tuple(str(col) for col in identity)
        published_columns = frozenset(str(col) for col in published)
    ...
    return PublicationSet(tables=tables)
...
class PublishWorker:
    ...
    @classmethod
    def from_ledger(
        cls, path: str | None = None, *, clock: Callable[[], float] = time.monotonic
    ) -> "PublishWorker":
        return cls(publication_set=load_publication_set(path), clock=clock)
    ...
    def apply(
        ...
        *,
        ...
        resolved_schedule = (
            schedule if schedule is not None else load_publish_schedule(schedule_path)
        )
        ...
        for table_key, batch_index, batch_rows in planned:
            ...
            try:
                ...
                inserted = _coerce_count(outcome.get("inserted_count"))
                updated = _coerce_count(outcome.get("updated_count"))
...
def is_publish_schedule_active(schedule: Mapping[str, Any] | None) -> bool:
    ...
    if not isinstance(approval, Mapping):
        return False
    return approval.get("status") in _ACTIVE_SCHEDULE_STATUSES
...
def _project_tables(
    ...
    for entry in table_entries:
        ...
        for row in kept_rows:
            ...
            if not row_keys.issubset(pub_table.allowed_columns):
                return _ProjectionResult(
                    code=PUBLICATION_TARGET_NOT_ADMITTED,
                    tables_rows={},
                    table_previews=[],
