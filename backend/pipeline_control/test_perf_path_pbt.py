...
targets the canonical path confinement surface implemented in task 45,
...
IF AND ONLY IF any path is outside ``apps/web/performance/*``. The superseded
``backend/performance/*`` tree is therefore rejected for every artifact role.
"""
...
# canonical boundary rule directly from Requirements 3.6/3.9 so the test
...
def _expected_violation(raw_paths, budget_paths) -> bool:
    """True IFF any path escapes the canonical web performance tree."""

    for raw in raw_paths:
        if not _under_web(raw):
            return True
    for budget in budget_paths:
        if not _under_web(budget):
            return True
    return False
...
# every kind of escape (legacy backend path and outside-root path) appears with
# meaningful frequency for both raw and budget artifacts. Separators are
...
class PerformanceEvidencePathSeparationProperties(unittest.TestCase):
    ...
    # returns performance_evidence_path_violation IF AND ONLY IF a raw or budget
    # artifact path is outside apps/web/performance/*.
