...
핵심 불변식 (요구사항 15, 설계 C11 결정 흐름):
  ...
  3. Agent_Action_Allowlist(로컬 멱등 6종)와 **정확히** 일치하는 조치만 사람
     개별 승인 없이 수행한다. 불일치는 ``agent_action_not_allowlisted``
     (요구사항 15.3, 15.4).
  4. 고위험 부류(Hosted 쓰기·마이그레이션·배포·롤백·브랜치 보호·시크릿·DNS)는
...
class Approval:
    ...
    def binds(self, trigger_signal_id: str, action_kind_id: str) -> bool:
        ...
        return (
            isinstance(self.approver_name, str)
    ...
    def _evaluate_action(
        ...
        *,
        ...
        try:
            ...
            # 실행 부작용 실패는 결과 확인 실패로 이어진다. 예외 원문을 노출하지
            # 않고 확인 단계로 넘긴다(아래 verifier가 False를 반환하도록).
            pass
