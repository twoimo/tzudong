...
class AuthWorkflowBridgeContractTests(unittest.TestCase):
    ...
    def test_exact_workflow_signatures_and_private_bridge_are_fail_closed(self) -> None:
        self.assertIn("CREATE ROLE privacy_auth_bridge NOLOGIN", self.sql)
        ...
        self.assertEqual(self.sql.count("OWNER TO privacy_auth_bridge;"), 9)
        self.assertIn("g041_auth_workflow_owner_direct_auth_dependency_remains", self.sql)

    def test_bridge_helper_and_runtime_claim_matrix_are_not_public(self) -> None:
        self.assertIn("g041_auth_subject_exists", self.sql)
        self.assertIn("FROM PUBLIC, anon, authenticated, service_role, privacy_workflow_owner", self.sql)
        for assertion in (
            "missing claims did not fail closed",
            "malformed claims did not fail closed",
            "cross-user authenticated read succeeded",
