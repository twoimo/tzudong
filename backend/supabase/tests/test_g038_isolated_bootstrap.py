...
class ManifestTests(unittest.TestCase):
    ...
    def test_manifest_requires_exact_ordered_parser_blob_parts(self):
        ...
        parser = value["statementParser"]
        self.assertEqual(tuple(parser["tokenBlobParts"]), bootstrap.TOKEN_BLOB_PARTS)
        self.assertEqual(tuple(parser["stateBlobParts"]), bootstrap.STATE_BLOB_PARTS)
        for key in ("tokenBlobParts", "stateBlobParts"):
            ...
            for replacement in malformed:
                with self.subTest(key=key, replacement=replacement):
                    ...
                    changed["statementParser"][key] = replacement
                    raw = json.dumps(changed, sort_keys=True, separators=(",", ":")).encode("ascii") + b"\n"
                    with self.assertRaises(RuntimeError):
                        bootstrap.manifest_inventory(raw)
        for key in ("tokenBlobParts", "stateBlobParts"):
            with self.subTest(missing=key):
                changed = json.loads(manifest())
                changed["statementParser"].pop(key)
                raw = json.dumps(changed, sort_keys=True, separators=(",", ":")).encode("ascii") + b"\n"
                with self.assertRaises(RuntimeError):
                    bootstrap.manifest_inventory(raw)
        changed = json.loads(manifest())
        changed["statementParser"]["tokenBlob"] = changed["statementParser"].pop("tokenBlobParts")
        raw = json.dumps(changed, sort_keys=True, separators=(",", ":")).encode("ascii") + b"\n"
        with self.assertRaises(RuntimeError):
            bootstrap.manifest_inventory(raw)
