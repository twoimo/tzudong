-- Add bounded public-profile read RPCs without reopening direct profiles access.

BEGIN;

SELECT pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended(
    'tzudong:local-profile-read-boundary:v1',
    0
  )
);
...
BEGIN
  IF pg_catalog.to_regrole('postgres') IS NULL
    ...
    RAISE EXCEPTION 'local_profile_read_boundary_prerequisite_missing';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'local_profile_read_boundary_identity_already_exists';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'local_profile_read_boundary_allowlist_conflict';
  END IF;
  ...
  IF pg_catalog.has_table_privilege(
    ...
    RAISE EXCEPTION 'local_profile_read_boundary_table_acl_drift';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'local_profile_read_boundary_column_drift';
  END IF;
END
$prerequisites$;
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'local_profile_read_boundary_owner_membership_drift';
  END IF;
  ...
  IF NOT v_membership_exists THEN
    ...
    PERFORM pg_catalog.set_config(
      'g014_006.remove_owner_membership', 'true', true
    );
    PERFORM pg_catalog.set_config(
      'g014_006.restore_owner_set_false', 'false', true
    );
  ELSIF v_supports_set_option AND NOT v_set_option THEN
    ...
    PERFORM pg_catalog.set_config(
      'g014_006.remove_owner_membership', 'false', true
    );
    PERFORM pg_catalog.set_config(
      'g014_006.restore_owner_set_false', 'true', true
    );
  ELSE
    PERFORM pg_catalog.set_config(
      'g014_006.remove_owner_membership', 'false', true
...
BEGIN
  ...
  IF v_rows <> 4 THEN
    RAISE EXCEPTION 'local_profile_read_boundary_allowlist_insert_drift';
  END IF;
END
$allowlist_insert$;
...
BEGIN
  IF v_summary IS NULL
    ...
    RAISE EXCEPTION 'local_profile_read_boundary_function_metadata_drift';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'local_profile_read_boundary_result_shape_drift';
  END IF;
  ...
  IF pg_catalog.has_function_privilege(
    ...
    RAISE EXCEPTION 'local_profile_read_boundary_function_acl_drift';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION 'local_profile_read_boundary_allowlist_readback_drift';
  END IF;
END
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'local_profile_read_boundary_owner_membership_cleanup_drift';
  END IF;
...
BEGIN
  ...
  IF v_asserted IS DISTINCT FROM true THEN
    RAISE EXCEPTION 'local_profile_read_boundary_catalog_assertion_failed';
  END IF;
END
