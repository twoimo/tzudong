...
CREATE TABLE IF NOT EXISTS local_analytics.publish_jobs (
    ...
    result_code    text CHECK (
        result_code IS NULL OR result_code IN (
            ...
            'preview_expired', 'batch_upsert_limit',
            'publish_readback_mismatch', 'publish_apply_aborted',
            'publish_schedule_not_approved', 'preview_generated',
            'confirm_admitted', 'apply_completed', 'converged_no_op',
            'readback_matched', 'publish_succeeded'
        )
    ),
...
CREATE TABLE IF NOT EXISTS local_analytics.publish_history (
    ...
    stage            text        NOT NULL
        CHECK (stage IN ('preview', 'confirm', 'apply', 'readback')),
    ...
    CONSTRAINT publish_history_counts_nonnegative CHECK (
        insert_row_count >= 0 AND update_row_count >= 0 AND total_row_count >= 0
        AND (batch_index IS NULL OR batch_index >= 0)
        AND (readback_rows IS NULL OR readback_rows >= 0)
        AND (matched_rows IS NULL OR matched_rows >= 0)
        AND (mismatched_rows IS NULL OR mismatched_rows >= 0)
    ),
    CONSTRAINT publish_history_preview_hash_shape CHECK (
...
REVOKE ALL ON ALL SEQUENCES IN SCHEMA local_analytics FROM PUBLIC, anon, authenticated;
