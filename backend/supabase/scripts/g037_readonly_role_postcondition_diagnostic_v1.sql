-- G037 rollback-only fixed-code role postcondition diagnostic.
--
...
BEGIN
  ...
  IF role_oid IS NULL THEN
    RAISE EXCEPTION 'g037_diag_role_flags_not_exact';
  END IF;
  ...
  IF target_function IS NULL THEN
    RAISE EXCEPTION 'g037_diag_target_function_missing';
  END IF;
  IF role_settings IS DISTINCT FROM ARRAY[
    ...
    RAISE EXCEPTION 'g037_diag_role_settings_not_exact';
  END IF;
  IF direct_database_privileges IS DISTINCT FROM ARRAY['CONNECT'] THEN
    RAISE EXCEPTION 'g037_diag_database_privileges_not_exact';
  END IF;
  IF direct_schema_privileges IS DISTINCT FROM ARRAY['USAGE'] THEN
    RAISE EXCEPTION 'g037_diag_schema_privileges_not_exact';
  END IF;
  IF direct_column_grants <> 3 THEN
    RAISE EXCEPTION 'g037_diag_column_grants_not_exact';
  END IF;
  IF unexpected_direct_acl_grants <> 0 THEN
    RAISE EXCEPTION 'g037_diag_unexpected_direct_acl';
  END IF;
  IF owned_object_count <> 0 THEN
    RAISE EXCEPTION 'g037_diag_owned_objects_not_zero';
  END IF;
  IF membership_count <> 0 THEN
    RAISE EXCEPTION 'g037_diag_memberships_not_zero';
  END IF;
  IF NOT pg_catalog.has_database_privilege(
    ...
    RAISE EXCEPTION 'g037_diag_database_connect_missing';
  END IF;
  IF pg_catalog.has_database_privilege(
    ...
    RAISE EXCEPTION 'g037_diag_database_create_present';
  END IF;
  IF pg_catalog.has_schema_privilege(
    ...
    RAISE EXCEPTION 'g037_diag_public_schema_create_present';
  END IF;
  IF NOT pg_catalog.has_column_privilege(
    ...
    RAISE EXCEPTION 'g037_diag_version_select_missing';
  END IF;
  IF NOT pg_catalog.has_column_privilege(
    'tzudong_g037_readonly',
    'supabase_migrations.schema_migrations',
    ...
    RAISE EXCEPTION 'g037_diag_name_select_missing';
  END IF;
  IF NOT pg_catalog.has_column_privilege(
    'tzudong_g037_readonly',
    'supabase_migrations.schema_migrations',
    ...
    RAISE EXCEPTION 'g037_diag_statements_select_missing';
  END IF;
  IF pg_catalog.has_function_privilege(
    ...
    RAISE EXCEPTION 'g037_diag_target_execute_present';
  END IF;
  RAISE EXCEPTION 'g037_diag_all_conditions_passed';
END;
$g037_postcondition$;
