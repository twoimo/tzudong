...
catalog_rows(kind,body) AS (SELECT 'schema',n.nspname||'|'||pg_get_userbyid(n.nspowner)||'|'||coalesce((SELECT string_agg(acl.grantor::text||chr(30)||acl.grantee::text||chr(30)||acl.privilege_type||chr(30)||acl.is_grantable::text,chr(29) ORDER BY acl.grantor,acl.grantee,acl.privilege_type,acl.is_grantable) FROM aclexplode(coalesce(n.nspacl,acldefault('n',n.nspowner))) AS acl),'') FROM pg_namespace n WHERE n.nspname='privacy_retention' UNION ALL SELECT 'relation',n.nspname||'.'||c.relname||'|'||c.relkind::text||'|'||pg_get_userbyid(c.relowner)||'|'||c.relrowsecurity||'|'||c.relforcerowsecurity||'|'||coalesce((SELECT string_agg(acl.grantor::text||chr(30)||acl.grantee::text||chr(30)||acl.privilege_type||chr(30)||acl.is_grantable::text,chr(29) ORDER BY acl.grantor,acl.grantee,acl.privilege_type,acl.is_grantable) FROM aclexplode(coalesce(c.relacl,acldefault('r',c.relowner))) AS acl),'') FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='privacy_retention' UNION ALL SELECT 'column',n.nspname||'.'||c.relname||'|'||a.attnum||'|'||a.attname||'|'||format_type(a.atttypid,a.atttypmod)||'|'||a.attnotnull||'|'||a.attidentity::text||'|'||a.attgenerated::text||'|'||coalesce(pg_get_expr(d.adbin,d.adrelid,true),'') FROM pg_attribute a JOIN pg_class c ON c.oid=a.attrelid JOIN pg_namespace n ON n.oid=c.relnamespace LEFT JOIN pg_attrdef d ON d.adrelid=a.attrelid AND d.adnum=a.attnum WHERE n.nspname='privacy_retention' AND a.attnum>0 AND NOT a.attisdropped UNION ALL SELECT 'constraint',n.nspname||'.'||c.relname||'|'||x.conname||'|'||x.contype::text||'|'||pg_get_constraintdef(x.oid,true) FROM pg_constraint x JOIN pg_class c ON c.oid=x.conrelid JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='privacy_retention' UNION ALL SELECT 'index',n.nspname||'.'||ci.relname||'|'||pg_get_indexdef(i.indexrelid) FROM pg_index i JOIN pg_class ct ON ct.oid=i.indrelid JOIN pg_namespace n ON n.oid=ct.relnamespace JOIN pg_class ci ON ci.oid=i.indexrelid WHERE n.nspname='privacy_retention' UNION ALL SELECT 'function',a.nspname||'.'||a.proname||'('||a.args||')|'||pg_get_userbyid(p.proowner)||'|'||p.prosecdef||'|'||p.provolatile::text||'|'||p.proparallel::text||'|'||coalesce(p.proconfig::text,'')||'|'||coalesce((SELECT string_agg(acl.grantor::text||chr(30)||acl.grantee::text||chr(30)||acl.privilege_type||chr(30)||acl.is_grantable::text,chr(29) ORDER BY acl.grantor,acl.grantee,acl.privilege_type,acl.is_grantable) FROM aclexplode(coalesce(p.proacl,acldefault('f',p.proowner))) AS acl),'')||'|'||pg_get_functiondef(p.oid) FROM actual_functions a JOIN pg_proc p ON p.oid=a.oid UNION ALL SELECT 'trigger',n.nspname||'.'||c.relname||'|'||t.tgname||'|'||t.tgenabled::text||'|'||pg_get_triggerdef(t.oid,true) FROM pg_trigger t JOIN pg_class c ON c.oid=t.tgrelid JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='privacy_retention' AND NOT t.tgisinternal UNION ALL SELECT 'default_acl',pg_get_userbyid(d.defaclrole)||'|'||d.defaclobjtype::text||'|'||coalesce(d.defaclacl::text,'') FROM pg_default_acl d JOIN pg_namespace n ON n.oid=d.defaclnamespace WHERE n.nspname='privacy_retention'),
