...
def manifest_inventory(raw: bytes) -> tuple[str, ...]:
    try:
        ...
        if (payload["schema"] != "g038-account-deletion-successor-v1"
                ...
                or set(parser) != {"schema", "stateBlob", "statePath", "tokenBlob", "tokenPath", "upstreamCommit", "upstreamVersion"}
                ...
                or not COMMIT_RE.fullmatch(parser["tokenBlob"])
                or not COMMIT_RE.fullmatch(parser["stateBlob"])):
            fail()
