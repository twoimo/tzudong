...
Each clone runs as the image's non-root `postgres` user with every Linux capability dropped and `no-new-privileges`. Its dedicated bridge disables both IP masquerading and inter-container communication; the adapter rejects any other network options. This egress-disabled bridge is required on Darwin because an engine `--internal` bridge suppresses the loopback port publication needed by the host-only restore client. The only admitted endpoint is the engine-assigned port published on `127.0.0.1`, read back from live container state; no caller chooses a port.
