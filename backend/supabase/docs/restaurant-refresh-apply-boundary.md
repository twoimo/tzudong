...
- The CLI-named SQL is now held in `backend/supabase/candidates/`, outside the canonical migration stream. It is not installed or counted as applied during a clean replay. The candidate starts with an **unconditional `refresh_apply_catalog_binding_pending` exception**. It is intentionally not deployable and is not a completed hosted migration. Keeping it outside `migrations` prevents an unconditional error from breaking source replay or promotion CI. Private tests remove only that exact block in a disposable database. No environment flag, session setting, or production bypass is supplied.
...
4. After exact catalog binding, generate a fresh CLI migration name and admit the reviewed candidate to the canonical migration stream; do not revive an unbound file solely to satisfy CI. Deploy database capability before enabling this route change. With the current pending gate there is no production admission. Apply only via parent's reviewed one-shot rehearsal/apply/readback workflow.
