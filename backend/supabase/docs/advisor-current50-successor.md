...
1. Retain the separately obtained exact-main runtime-probe receipt. Its expected
   ...
   historical ledger or closure. Resolve any active G037/G038 freeze before this
   separate successor operation; do not run it inside another controller window.
...
3. Generate `snapshot` SQL and have the parent run it read-only on that exact
   target. Retain only the one JSON object, with no CLI wrapper, as snapshot JSON.
...
CLI 2.115.0 `db query --project-ref aqlcofblfxdrjhhdmarw --file ...` route, but must
...
All 26 functions remain invoker functions with the exact fixed path. Their other
catalog attributes, including bodies, owners and ACLs, remain identical. The
hosted-only touch function must already have the exact recovered body, postgres
owner and owner-only ACL; unexpected normalization is refused rather than hidden.
