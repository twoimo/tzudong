...
G040 preserves the hosted `vector` extension in `public` on both restored clones. Its executable G014 vectors replace exactly eight source-pinned `extensions.vector` identity literals (four in `20260713002000`, four in `20260713002400`) with `public.vector`; any missing or additional occurrence blocks compilation. The same transform changes exactly one G014 creator predicate so `supabase_admin` is accepted only for a `public` function that has an exact extension-membership dependency on the `public`-schema `vector` extension owned by `supabase_admin`; unrelated provider-owned functions remain rejected. This compatibility vector is identical in both clone branches and hosted execution, while canonical migration bytes remain unchanged in ledger evidence.
