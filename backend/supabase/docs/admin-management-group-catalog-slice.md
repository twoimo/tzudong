...
3. The shared generator/workflow now include the group overlap branch, both test modules, path filters, and the explicit private-PG opt-in. The migration remains in the canonical source chain. Verifier, planner, advisor utility, and parser are required clean inputs and individually hashed into that chain; generated verification SQL and its validated receipt are retained in the artifact manifest. The final actual G014 assertion remains unchanged. Node setup precedes source tests because source contracts invoke the canonical parser. Run the full dual replay on the exact integration commit; skipped database tests in the source-only step are not private-PG evidence.
