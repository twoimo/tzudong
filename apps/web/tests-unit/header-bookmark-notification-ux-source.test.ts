...
describe('header bookmark and notification UX source contracts', () => {
  test('desktop bookmark menu has intent-loaded data, responsive panel, and clear loading/empty/error states', () => {
    ...
    expect(hookSource).toContain('export function useUserBookmarkCount');
    expect(hookSource).toContain("queryKey: ['user-bookmark-count', user?.id]");
    expect(hookSource).toContain(".select('id', { count: 'exact', head: true })");
    ...
    expect(bookmarkSource).toContain('useUserBookmarkCount()');
    expect(bookmarkSource).toContain('const triggerBookmarkCount = bookmarkCount ?? bookmarksData.length;');
  ...
  test('desktop notification menu distinguishes loading, error, empty, unread, and destination states', () => {
    ...
    expect(headerSource).toContain('event.stopPropagation();');
    expect(headerSource).toContain('onPointerDown={(event) => event.stopPropagation()}');
    expect(headerSource).toContain('알림 삭제');
    expect(headerSource).toContain('className="sr-only">알림 삭제</span>');
    expect(headerSource).toContain('flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-muted-foreground hover:bg-destructive/10 hover:text-destructive');
    ...
    expect(headerSource).toContain('flex w-full max-w-full cursor-pointer items-center gap-2 rounded-xl p-2.5');
  ...
  test('mobile bookmark and notification controls keep the same touch, state, and responsive affordances', () => {
    ...
    expect(mobileBookmarkSource).not.toContain('rounded-xl bg-muted/40 px-3 py-2.5 text-foreground');
    ...
    expect(mobileNotificationSource).toContain('flex items-start justify-between gap-3 px-1 py-1 text-foreground');
    ...
    expect(mobileNotificationSource).toContain('event.stopPropagation();');
    expect(mobileNotificationSource).toContain('onPointerDown={(event) => event.stopPropagation()}');
    expect(mobileNotificationSource).toContain('알림 삭제');
    expect(mobileNotificationSource).toContain('className="sr-only">알림 삭제</span>');
    expect(mobileNotificationSource).toContain('flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-muted-foreground hover:bg-destructive/10 hover:text-destructive');
    expect(mobileNotificationSource).not.toContain('ml-1 mt-1 flex cursor-pointer items-center gap-2 rounded-xl px-3 py-2');
    expect(mobileNotificationSource).not.toContain('Fragment key={notification.id}');
