...
describe('home root runtime boundary', () => {
    test('keeps the root first paint static while preserving the real home runtime frame', () => {
        ...
        expect(publicStaticSource).toContain('<h1 class="title">지도를 준비하고 있어요</h1>');
        expect(publicStaticSource).toContain('class="status-card"');
        ...
        expect(publicStaticSource).toContain('지도를 준비하고 있어요');
        expect(publicStaticSource).toContain('지도 화면을 먼저 준비하고 맛집 정보를 순서대로 불러옵니다');
        ...
        expect(publicStaticSource).toContain('@media (prefers-reduced-motion:reduce)');
        expect(publicStaticSource).toContain('.step-dot.active{animation:none!important}');
