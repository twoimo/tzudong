...
describe('web quality performance source contracts', () => {
    ...
    test('home map runtime renders directly while supporting queries stay intent-gated', () => {
        ...
        const staticHomeSource = source('public/home-static.html');
        ...
        expect(staticHomeSource).toContain('id="home-initial-shell"');
        expect(staticHomeSource).toContain('role="status" aria-live="polite"');
        expect(staticHomeSource).toContain('aria-label="쯔동여지도 홈 미리보기"');
        expect(staticHomeSource).toContain('aria-live="polite"');
        expect(staticHomeSource).toContain('class="sr"');
        expect(staticHomeSource).toContain('.sr{position:absolute;width:1px;height:1px');
        expect(staticHomeSource).toContain('clip:rect(0,0,0,0)');
        expect(staticHomeSource).toContain('white-space:nowrap');
        expect(staticHomeSource).toContain('class="map-surface"');
        expect(staticHomeSource).not.toContain('class="loader"');
        expect(staticHomeSource).not.toContain('지도를 준비하고 있어요');
        expect(staticHomeSource).not.toContain('class="status-card"');
        expect(staticHomeSource).not.toContain('class="loader-wrap"');
        expect(staticHomeSource).not.toContain('.card{padding:28px 32px}');
        expect(staticHomeSource).toContain('class="search-shell"');
        expect(staticHomeSource).toContain('data-home-intent="search"');
        expect(staticHomeSource).toContain('data-home-intent="bookmark"');
        expect(staticHomeSource).toContain('min-height:44px');
        expect(staticHomeSource).toContain('쯔동여지도 검색 열기');
        expect(staticHomeSource).toContain('tzudong:home-initial-intent');
        expect(staticHomeSource).not.toContain('background:linear-gradient(90deg');
        expect(staticHomeSource).not.toContain('지도를 준비하고 있어요');
        expect(staticHomeSource).toContain('property="og:image"');
        expect(staticHomeSource).toContain('name="twitter:card"');
        expect(staticHomeSource).toContain('rel="icon"');
        expect(staticHomeSource).toContain('rel="preconnect" href="https://oapi.map.naver.com"');
        expect(staticHomeSource).toContain('rel="dns-prefetch" href="https://img.youtube.com"');
        expect(staticHomeSource).not.toContain('@media (prefers-reduced-motion:reduce)');
        expect(staticHomeSource).not.toContain('.step-dot.active');
        expect(staticHomeSource).not.toContain('.spin.secondary');
        expect(staticHomeSource).toContain("frame.id='home-runtime-frame'");
        ...
        expect(homeClientSource).toContain('홈 지도 화면 준비 중');
        ...
        expect(homeControlPanelSource).toContain('useState(Boolean(initialIntent))');
        ...
        expect(homeControlPanelSource).toContain('return <DesktopControlPanelLoadingShell />;');
        ...
        expect(homeDesktopControlPanelSource).toContain('setShouldLoadSearch(true)');
