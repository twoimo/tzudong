...
describe('web quality performance source contracts', () => {
    ...
    test('home map runtime renders directly while supporting queries stay intent-gated', () => {
        ...
        expect(staticHomeSource).toContain('aria-label="쯔동여지도 로딩 중"');
        expect(staticHomeSource).toContain('class="map-surface"');
        expect(staticHomeSource).not.toContain('class="loader"');
        expect(staticHomeSource).toContain('<h1 class="title">지도를 준비하고 있어요</h1>');
        expect(staticHomeSource).toContain('class="status-card"');
        ...
        expect(staticHomeSource).toContain('지도를 준비하고 있어요');
        ...
        expect(staticHomeSource).toContain('@media (prefers-reduced-motion:reduce)');
        expect(staticHomeSource).toContain('.step-dot.active{animation:none!important}');
        ...
        expect(source('app/home-initial-shell.tsx')).toContain('지도를 준비하고 있어요');
        expect(source('app/home-initial-shell.tsx')).toContain('지도 화면을 먼저 준비하고 맛집 정보를 순서대로 불러옵니다');
        expect(source('app/home-initial-shell.tsx')).not.toContain('bg-gradient-to-r');
        expect(source('app/home-initial-shell.tsx')).toContain('motion-reduce:animate-none');
        expect(source('app/home-initial-shell.tsx')).not.toContain('motion-reduce:hidden');
        expect(source('app/home-initial-shell.tsx')).toContain('홈 지도 준비 단계');
        ...
        expect(homeRuntimeShellSource).toContain('지도를 준비하고 있어요');
        expect(homeRuntimeShellSource).toContain('지도 화면을 먼저 준비하고 맛집 정보를 순서대로 불러옵니다');
        ...
        expect(homeRuntimeShellSource).toContain('홈 지도 준비 단계');
    ...
    test('page-level loaders keep fullscreen opt-in while map fallbacks stay embedded', () => {
        ...
        expect(mapSkeletonSource).toContain('지도 화면을 준비하고 있어요');
