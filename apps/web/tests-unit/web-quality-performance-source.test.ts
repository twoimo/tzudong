...
describe('web quality performance source contracts', () => {
    ...
    test('home map runtime renders directly while supporting queries stay intent-gated', () => {
        ...
        expect(mapIndicatorsSource).toContain('role="status"');
        expect(mapIndicatorsSource).toContain('aria-live="polite"');
        ...
        expect(mapViewIndicatorsSource).toContain('aria-busy="true"');
        ...
        expect(overlayStackSource).toContain("aria-live={mapToast.type === 'error' ? 'assertive' : 'polite'}");
