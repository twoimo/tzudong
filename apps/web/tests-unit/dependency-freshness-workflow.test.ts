...
describe('Dependency_Freshness_Workflow governance contract', () => {
  ...
  test('all seven units mirror the current dependabot.yml directories', () => {
  ...
  test('targets develop only and never auto-merges', () => {
    ...
    expect(targets.length).toBe(UNITS.length);
  });
  ...
  test('keeps at most five open candidates per unit', () => {
    ...
    expect(limits.length).toBe(UNITS.length);
  ...
  test('pins the current Node, npm, and Bun release authorities', () => {
    expect(workflow).toContain("NODE_VERSION: '24'");
    expect(workflow).toContain("NPM_AUTHORITY: '11.6.2'");
    expect(workflow).toContain("BUN_VERSION: '1.4.0'");
    expect(workflow).toContain('bun-version: ${{ env.BUN_VERSION }}');
    expect(workflow).toContain('npm install --global npm@"${NPM_AUTHORITY}"');
    expect(workflow).not.toContain('/backend/deploy/pipeline-control');
  });
});
