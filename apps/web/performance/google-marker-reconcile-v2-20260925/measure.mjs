...
try {
    ...
    await page.route('http://127.0.0.1/**', (route) => route.request().resourceType() === 'image' ? route.fulfill({
        contentType: 'image/svg+xml',
        body: '<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"><circle cx="16" cy="16" r="14" fill="orange"/></svg>',
    }) : route.fulfill({
    ...
    await page.evaluate(async (code) => {
        ...
        globalThis.preloadMarkerFixture = async () => {
            const f = fixture(1, 1);
            const image = f.container.querySelector('img');
            await image.decode();
            const imageDecoded = image.complete && image.naturalWidth === 32;
            f.dispose();
            if (!imageDecoded) throw new Error('The marker fixture image did not decode');
            return imageDecoded;
        };
    ...
    const imageDecoded = await page.evaluate(() => globalThis.preloadMarkerFixture());
    const verification = { ...await page.evaluate(() => globalThis.checkMarkers()), imageDecoded };
