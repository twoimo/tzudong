...
if (Number(process.versions.node.split('.')[0]) !== 24) throw new Error('Use the project-pinned Node 24 runtime.');
...
const scenarios = [
    ...
    { name: 'revisit-64-searches', size: 200, iterations: 20000, searchCount: 64 },
    { name: 'revisit-128-searches-over-capacity', size: 200, iterations: 10000, searchCount: 128 },
];
...
try {
    ...
    await page.route('http://127.0.0.1/', (route) => route.fulfill({
        ...
        body: '<!doctype html><title>Local helper benchmark</title>',
    }));
    await page.goto('http://127.0.0.1/');
    const isolatedTimer = await page.evaluate(() => crossOriginIsolated);
    if (!isolatedTimer) throw new Error('High-resolution isolated timer is unavailable');
    await page.evaluate(async (moduleSources) => {
        ...
        const prepare = (scenario, module) => {
            ...
            const queryCount = scenario.name === 'same-search-cache-hit' ? 1
                : scenario.searchCount ?? scenario.iterations;
            ...
            if (scenario.name === 'same-search-cache-hit' || scenario.searchCount) {
                const count = scenario.name === 'same-search-cache-hit' ? 1 : scenario.searchCount;
        ...
        const inputAt = (state, scenario, index) => ({
            ...
            activeSearchedRestaurant: state.queries[
                scenario.name === 'same-search-cache-hit' ? 0 : scenario.searchCount ? index % scenario.searchCount : index
            ],
        });
    ...
    const raw = {
        ...
        environment: { node: process.version, chromium: browser.version(), platform: platform(), arch: arch(), cpu: cpus()[0]?.model, crossOriginIsolated: isolatedTimer },
