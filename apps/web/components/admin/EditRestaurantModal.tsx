...
export function EditRestaurantModal({ record, open, onOpenChange, onSuccess }: EditRestaurantModalProps) {
  ...
  const geocodeWithMapbox = async (query: string, limit: number = 3): Promise<Array<{
    ...
    try {
      ...
      const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodedQuery}.json?access_token=${accessToken}&limit=${limit}&language=ko,en`;

      const response = await fetch(url);

      if (!response.ok) {
        // 422 Unprocessable Entity: 쿼리가 너무 길거나 형식이 잘못됨 (빈 배열 반환하여 다음 시도 진행)
