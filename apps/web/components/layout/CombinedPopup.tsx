...
const CombinedPopupComponent = () => {
    ...
    // 팝업 표시
    useEffect(() => {
        if (!canLoadBanners || posterBanners.length === 0 || hasShownRef.current) return;
    ...
    useEffect(() => {
        if (!isAutoPlaying || !isVisible || posterBanners.length <= 1) return;
