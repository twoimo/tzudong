...
export async function POST(_request: NextRequest, context: TrendJobRequestRouteContext) {
  ...
  try {
    ...
    if (currentError) return noStoreJson({ ok: false, error: 'trend_job_request_status_failed' }, { status: 502 });
    const current = readRowObject(currentData);
    if (!current) return noStoreJson({ ok: false, error: 'trend_job_request_not_found' }, { status: 404 });
    if (current.status === 'claimed') return noStoreJson({ ok: false, error: 'trend_job_request_already_claimed' }, { status: 409 });
    if (current.status === 'succeeded' || current.status === 'failed' || current.status === 'cancelled') {
      return noStoreJson({ ok: false, error: 'trend_job_request_terminal' }, { status: 409 });
    }
    ...
    if (error) {
      const { data: racedData } = await supabase
        ...
        .eq('requested_by_admin_id', admin.userId)
        .maybeSingle();
      const raced = readRowObject(racedData);
      if (raced?.status === 'claimed') {
        return noStoreJson({ ok: false, error: 'trend_job_request_already_claimed' }, { status: 409 });
      }
    ...
    return noStoreJson({ ok: true, request: mapTrendJobRequestRow(readRowObject(data) ?? {}) });
  } catch {
    return noStoreJson({ ok: false, error: 'trend_job_request_status_failed' }, { status: 502 });
  }
}
