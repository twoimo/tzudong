...
export async function POST(request: NextRequest, context: StoryboardJobCancelRouteContext) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) throw new Error('storyboard_job_cancel_failed');
    ...
    const { data: latest, error: readError } = await supabase
      ...
      .eq('requested_by_admin_id', auth.userId)
      .maybeSingle();
    if (readError) throw new Error('storyboard_job_cancel_failed');
    ...
    console.error('[admin/storyboard/jobs] boundary failed', sanitizePrivacyValue({
      ...
      code: 'storyboard_job_cancel_failed',
      traceId,
    }).value);
    return noStoreJson(telemetry, { ok: false, error: 'storyboard_job_cancel_failed', traceId }, { status: 502 });
  }
}
