...
async function createChallenge(input: NonNullable<ReturnType<typeof parseOnboardingStart>>) {
  ...
  if (error || !result
    || typeof result.challengeId !== 'string' || !UUID_PATTERN.test(result.challengeId)
    || result.policyVersionId !== input.policyVersion
    || result.ageBand !== input.ageBand
...
async function confirmChallenge(
  ...
  const result = getResultRecord(data);
  const readback = result && isRecord(result.readback) ? result.readback : null;
  if (error || !result
    || result.schemaVersion !== 1
    || result.operationId !== payload.challengeId
    || result.challengeId !== payload.challengeId
    || result.userId !== userId
    || result.policyVersionId !== payload.policyVersionId
