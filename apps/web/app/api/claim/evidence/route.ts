...
import { submitRestaurantClaimEvidence } from '@/lib/claim/store';
...
export async function POST(request: NextRequest) {
  ...
  if (!isPlainObject(bounded.value) || !hasExactKeys(bounded.value, ['claimId', 'evidence'])) {
    return claimErrorResponse(RESTAURANT_CLAIM_ERROR.invalidRequest, 400);
  }
  const claimId = bounded.value.claimId;
  const evidence = bounded.value.evidence;
  if (!isUuid(claimId) || !isPlainObject(evidence)) {
    return claimErrorResponse(RESTAURANT_CLAIM_ERROR.invalidRequest, 400);
  }

  try {
    assertPrivacySafe({ claimId, evidence });
  } catch (error) {
  ...
  const result = submitRestaurantClaimEvidence({
    claimId,
    userId: auth.userId,
    evidence,
  });
