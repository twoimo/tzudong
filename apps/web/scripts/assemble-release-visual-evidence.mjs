...
const projectRoot = realpathSync(path.resolve(directory, '..'));
...
function safeFile(value, root, label, maxBytes) { const lexical = path.resolve(value); let canonicalParent; try { canonicalParent = realpathSync(path.dirname(lexical)); } catch { fail(`${label} is missing`); } const canonical = path.join(canonicalParent, path.basename(lexical)); if (!inside(canonical, root)) fail(`${label} escapes its directory`); let link; try { link = lstatSync(canonical); } catch { fail(`${label} is missing`); } if (link.isSymbolicLink() || !link.isFile() || link.nlink !== 1) fail(`${label} must be a single-link non-symlink regular file`); if (!inside(realpathSync(canonical), root)) fail(`${label} escapes its directory`); const flags = constants.O_RDONLY | (constants.O_NOFOLLOW ?? 0); let fd; try { fd = openSync(canonical, flags); } catch { fail(`${label} cannot be opened without following links`); } try { const stat = fstatSync(fd); if (!stat.isFile() || stat.nlink !== 1 || stat.dev !== link.dev || stat.ino !== link.ino || !Number.isSafeInteger(stat.size) || stat.size > maxBytes) fail(`${label} changed or exceeds size limit`); const bytes = Buffer.alloc(maxBytes + 1); let offset = 0; while (offset <= maxBytes) { const count = readSync(fd, bytes, offset, maxBytes + 1 - offset, offset); if (!count) break; offset += count; } const after = fstatSync(fd); if (offset > maxBytes || after.dev !== stat.dev || after.ino !== stat.ino || after.nlink !== 1 || after.size !== stat.size || after.size !== offset) fail(`${label} changed or exceeds size limit`); return bytes.subarray(0, offset); } finally { closeSync(fd); } }
...
export function assembleReleaseVisualEvidence(argv, issuer) {
    ...
    const cellsDirectory = safeDirectory(cli.values.get('--cells'), 'cells directory', true);
    const requestedOutput = cli.values.get('--output');
    const lexicalOutputPath = path.resolve(requestedOutput);
    if (!path.isAbsolute(requestedOutput) || inside(lexicalOutputPath, projectRoot) || lstatSync(path.dirname(lexicalOutputPath)).isSymbolicLink()) fail('output path must be external with a non-symlink parent');
    const outputDirectory = safeDirectory(path.dirname(lexicalOutputPath), 'output directory', true);
    const outputPath = path.join(outputDirectory, path.basename(lexicalOutputPath));
    const outputIdentity = lstatSync(outputDirectory);
    if (pathExistsSafely(outputPath, 'output')) fail('output must be create-only');
    const artifactsDirectory = path.join(outputDirectory, 'artifacts');
    if (pathExistsSafely(artifactsDirectory, 'artifacts output')) fail('artifacts output must be create-only');
...
if (isMain) process.on('uncaughtException', (error) => {
    if (process.env.G009_DEBUG_ASSEMBLY === '1') process.stderr.write(`${error?.stack ?? error}\n`);
    process.stderr.write('RELEASE_VISUAL_ASSEMBLY_FAILED\n');
    process.exit(1);
});
if (isMain) {
