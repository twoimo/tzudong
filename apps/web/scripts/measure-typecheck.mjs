...
function isNoisy(profiles) { return Object.values(profiles).some((profile) => profile.durationMs.mad * 100 > profile.durationMs.median * 5 || profile.peakRssBytes.mad * 100 > profile.peakRssBytes.median * 5); }
...
async function main() {
  ...
  try {
    ...
    for (const kind of ['native', 'compat']) {
      ...
      outcomes.push({ phase: 'warmup', kind, rawOutput: path.basename(rawOutput), rawSha256, summary: result.samplerSummary });
      ...
      for (let position = runs.length; position < sequence.length; position += 1) {
        ...
        for (let retry = 0; retry <= 2; retry += 1) {
          ...
          try {
            ...
            const rawSha256 = await digestFile(rawOutput);
            outcomes.push({ phase: 'measured', position: position + 1, retry: retry + 1, kind, accepted: valid, rawOutput: path.basename(rawOutput), rawSha256, summary: result.samplerSummary, failure: valid ? null : 'COMPILER_FAILURE' });
            if (valid) {
            ...
            lastError = error;
            outcomes.push({ phase: 'measured', position: position + 1, retry: retry + 1, kind, accepted: false, rawOutput: path.basename(rawOutput), rawSha256: await exists(rawOutput) ? await digestFile(rawOutput) : null, failure: safeCliErrorName(error) });
          }
          invalidRuns[kind] += 1;
    ...
    let sequence = await executeSamples(7); let measured = profileStats(runs); const initialNoise = isNoisy(measured); if (initialNoise) { sequence = await executeSamples(9); measured = profileStats(runs); if (isNoisy(measured)) throw new Error('Persistent benchmark noise after nine samples requires Option A replanning'); }
    ...
    const durationMedianRegression = durationMedianDelta > 250 && durationMedianDelta / measured.compat.durationMs.median > 0.05;
    const durationP75Regression = durationP75Delta > 250 && durationP75Delta / measured.compat.durationMs.p75 > 0.10;
    const durationRegression = durationMedianRegression || durationP75Regression;
    const rssRegression = rssDelta > 128 * 1024 * 1024 && rssDelta / measured.compat.peakRssBytes.p95NearestRank > 0.15;
    ...
    const profileSupportsPositiveSpeedClaim = speedupMs >= 200 && speedupRatio >= 0.10 && speedupMs >= 2 * pooledMad;
    const report = {
      schemaVersion: 4,
      ...
      comparison: {
        ...
        positiveSpeedClaimProfileEvidence: {
          ...
          speedupRatio,
          pooledMad,
          eligible: profileSupportsPositiveSpeedClaim,
        },
      },
      acceptance: {
        ...
        durationRegression,
        rssRegression,
        passed: !durationRegression && !rssRegression,
      },
    };
    await writeFile(path.join(stage, 'report.json'), `${JSON.stringify(report, null, 2)}\n`, { flag: 'wx' }); if (await exists(publicationDirectory)) throw new Error('Publication destination appeared during staging'); await rename(stage, publicationDirectory); process.stdout.write(`${JSON.stringify({ status: report.acceptance.passed ? 'passed' : 'failed', output: path.basename(options.output), profiles: measured })}\n`); if (!report.acceptance.passed) process.exitCode = 1;
  } catch (error) {
