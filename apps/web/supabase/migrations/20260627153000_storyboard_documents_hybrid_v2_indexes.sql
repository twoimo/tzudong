-- Versioned storyboard document hybrid search optimization.
-- Keeps match_storyboard_documents_hybrid v1 live; routes can roll back with STORYBOARD_RAG_SEARCH_RPC_VERSION=v1.

create index if not exists documents_metadata_path_gin_idx
  on public.documents using gin(metadata jsonb_path_ops);

-- jsonb_path_ops is compact for @>, but the sparse candidate prefilter uses ?| key existence.
create index if not exists documents_sparse_lexical_weights_keys_gin_idx
  on public.documents using gin(sparse_lexical_weights);
...
comment on function public.match_storyboard_documents_hybrid_v2(uuid, vector(1024), jsonb, double precision, integer, integer, jsonb)
  is 'Storyboard RAG hybrid v2: keeps dense HNSW path and prefilters sparse JSONB candidates with ?| keys before sparse dot product.';
