...
- Primary product surfaces: public map/home shell, admin unified console, admin moderation/evaluations, submissions/reviews, banner management, insights, LLM operations session panel
- Evidence reviewed:
...
- Non-goals:
  ...
  - No Supabase schema/RLS/mutation behavior changes.
...
- Core routes/screens: `/admin`, `/admin/evaluations`, `/admin/banners`, `/admin/submissions`, `/insights`.
- Content hierarchy:
...
- New/changed components:
  ...
  - Admin overview hero: Korean-only operational copy, concise status chips, clear primary CTA.
  - Overview cards/guarded-apply panels: card/pill rhythm consistent with 맛집/제보/리뷰/배너 management surfaces.
...
## Interaction states
- Loading: use `GlobalLoader` for auth/page gates; inline admin modules should use task-shaped skeletons with Korean status copy, visible target queue/status chips, and guarded-action steps rather than a single “준비 중” sentence. For the overview “제보 큐 확인” transition, the loading state must explicitly say the right canvas is opening the 제보 큐 in-place, show queue/filter/detail-preview placeholders, and keep the Preview → Confirm → Apply → Readback → Audit safety model visible.
...
- Terminology: use “관리자 콘솔”, “운영 요약”, “읽기 전용”, “검토 필요”, “적용 전 확인”, “감사 기록”, “이 화면에서 열기”.
- Microcopy rules:
...
- Test/screenshot expectations:
  ...
  - AHP committee target for the admin console: weighted score must be 98+ before claiming the expert-committee UI loop complete.
...
- Required evidence for a 98+ claim: lint, typecheck, production build, authenticated browser smoke across overview plus all sidebar modules, no missing accessible names, no horizontal overflow, focus/scroll reset on module switch, URL-backed module state, no visible English-first chrome, and saved AHP/visual-verdict JSON.
