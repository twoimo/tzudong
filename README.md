...
- **영상 제목 검색**: 검색창에서 YouTube 영상 제목으로 실시간 필터링
...
3. **영상 제목 검색**: 검색창에 YouTube 영상 제목 입력하여 필터링
4. **컬럼 필터**: 각 컬럼 헤더의 메뉴 버튼(≡)으로 조건 설정
   - 필터 적용된 컬럼은 초록색 배경으로 표시됨
5. **필터 초기화**: 왼쪽 상단 새로고침 아이콘(↻) 버튼으로 모든 필터 리셋
6. **상세 보기**: 행 왼쪽 확장 버튼(▼) 클릭
7. **검토**: 7가지 평가 지표 확인 및 YouTube 영상 클릭하여 검증
8. **액션 실행**:
