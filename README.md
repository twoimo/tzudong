...
`/mypage/profile` kept its desktop cards inside `md:h-full md:min-h-0 md:grid-rows-2`, so two viewport rows split the scroller height while the password form needed 348px. At 1024x900 the 비밀번호 변경 card measured `clientHeight 203` against `scrollHeight 348` under `overflow: hidden`, which clipped the form inside the card. The page surface now keeps only `md:grid-cols-2 md:auto-rows-auto md:content-stretch md:items-stretch`: rows follow card content, the password card measures 306px with `clientHeight` equal to `scrollHeight`, and the grid box equals its scroll height (958 vs 956). `data-mypage-profile-viewport-fit` now reads `content` instead of `true`.
