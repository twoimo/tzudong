...
1.  **Vercel (Client)**: 하루 **20회**까지는 Vercel(Google API)에서 직접 처리하여 가장 빠릅니다.
2.  **OCI Server (Fallback)**:
    *   20회를 초과하거나, Vercel에서 에러가 발생하면 **즉시 OCI 서버로 전환**됩니다.
...
2.  **Supabase의 3가지 핵심 임무**:
    ...
    *   📊 **사용량 제한 (Quota)**: `ocr_logs` 테이블을 확인하여 **'하루 20회'** 무료 사용량을 체크합니다.
