...
- 데스크톱 왼쪽 피드 패널은 본문을 가리던 플로팅 `+`를 헤더 아이콘으로 옮겼다. 작성자는 그대로 `쯔동마스터`, `먹보쯔양팬`이다.
- 데스크톱 피드 헤더는 `리뷰 (2개)`와 아이콘 4개를 한 줄에 둔다. [화면](public-feed-header-single-row-desktop.png). 390px에서는 테마 칩을 가로 스크롤하고, 로그인 탭은 밑줄형(`94984d05`, [화면](public-login-final-390.png)), 상세 패널은 모바일에서 왼쪽 테두리를 제거했다. [지도](public-map-filter-reel-390.png) [피드](public-feed-final-390.png) [상세](public-detail-final-390.png).
...
- 로컬 스택(`tzudong-local-ab5bc03b76d9`, GoTrue `http://127.0.0.1:28000`)의 `auth.users`/`auth.identities`가 이전 복제에서 자격 증명과 신원을 비워 둔 상태였다. `twoimo@dgu.ac.kr`에는 로컬 재발급 해시 1건만 있었고 호스티드 해시와 달랐다.
...
- 이 단계는 저장소의 일반 호스티드→로컬 복제 계약(자격 증명·신원 제외)과 [privacy](../../agents/privacy.md)의 무기록 원칙을 개발자 로컬 스택에 한해 사용자 지시로 벗어난 것이다. 자격 증명 값은 이 기록에 남기지 않았고, 일반 클론 도구는 그대로 둔다.
- 브라우저 readback: 인앱 브라우저에서 `http://127.0.0.1:3000/mypage/profile`이 `쯔동마스터`·`twoimo@dgu.ac.kr`·도장 2·리뷰 7로 렌더링된다. 로컬 GoTrue가 발급한 세션을 앱의 `sb-127-auth-token(.0/.1)` 쿠키 형식으로 저장한 상태이며, `/mypage/profile`은 200으로 열리고 쿠키 없는 요청은 307로 `/auth/required`로 돌아간다.
