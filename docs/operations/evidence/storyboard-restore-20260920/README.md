...
- 데스크톱 피드 헤더는 `리뷰 (2개)`와 아이콘 4개를 한 줄에 둔다. [화면](public-feed-header-single-row-desktop.png). 390px에서는 테마 칩을 가로 스크롤하고, 로그인 탭은 밑줄형, 상세 패널은 모바일에서 왼쪽 테두리를 제거했다. [지도](public-map-filter-reel-390.png) [피드](public-feed-final-390.png) [상세](public-detail-final-390.png).
