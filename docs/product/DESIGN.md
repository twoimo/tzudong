...
- Recovery/release constraints: the dirty original worktree is immutable and the isolated recovery candidate is the only edit surface; never reset, stash, or clean. Start each serialized content-patch PR from a fresh head and promote `develop -> data -> main` under branch protection. The repository owner is the only operator; a second reviewer is not required.
- Hosting constraints: verify the exact Git-integrated `tzudong` Vercel project before an action; do not use a stale `web` project or mutate DNS. A release or rollback requires the operator's approval, branch-protection evidence, rollback planning, and a deployment readback receipt. Schema, RLS, retention, policy, marketing, location, under-14, incident, and hosted-catalog evidence gates are parked in [2026-09-22-parked-external-gates.md](../archive/release-gates/2026-09-22-parked-external-gates.md) and do not block release. No merge or deployment is asserted here.
...
- [ ] Production privacy migration/RLS/RPC/type readback / owner: backend/privacy owner / impact: parked on 2026-09-22; not a release blocker. Local schema tests still do not prove the deployed catalog.
...
- These interaction controls are product safeguards, not a legal-compliance certification. On 2026-09-22 the operator parked policy publication, retention periods, guardian approval, location-business filing, incident notices, marketing-provider approval, schema, RLS, and hosted-catalog evidence as release gates. Restore [2026-09-22-parked-external-gates.md](../archive/release-gates/2026-09-22-parked-external-gates.md) to enforce them again. The controls in this section stay in the product until that restore includes a deliberate product change.
