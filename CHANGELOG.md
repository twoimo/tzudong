...
- Evaluate skips YouTube IDs that already have local `selection`/`notSelection` files, so foreign videos that never reach hosted cannot consume the daily `--limit` slot forever.
